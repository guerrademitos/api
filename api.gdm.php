...
function getDecks(){
	...
	$query = db_select('mp_deck_cards','d');
	$query->fields('d', array('deckId','cardId','quantity'));
	$results = $query->execute();

	while($res = $results->fetchObject())
		$deckCards[] = new DeckCard($res);
    
	$query = db_select('mp_decks','d');
    $query->fields('d', array('id','basedIn','creator','description'));
	$decksResult = $query->execute();
    ...
    foreach($deckCards as $deckCard) {
        foreach($decks as $deck) {
            if($deckCard->deckId == $deck->id) $deck->deckCards[] = $deckCard;
        }
    }
...
function createDeck($deck){
    ...
    try {
        $id = db_insert('mp_decks')
        ->fields(array(
        'basedIn' => $deck->basedIn,
